...
void GoUp(int r, int c) {//r c의 칸을 가능한 한 상승시킨다
	if (!field_Ima[r][c] || r > N || c > N || r < 1 || c < 1)
		return;
	int rr = r; int cc = c;
	...
	else if (field_Ima[rr][cc]) {//부딪혀서 멈췄으면
		int rrr = rr - 1; int ccc = c;
		if (field_Ima[rrr][ccc] == field_Ima[rr][cc]) {//동일한 수면 병합한다
			field_Ima[rrr][ccc] *= 2;
			if (maxval < field_Ima[rrr][ccc])
...
void AllGo(int d) {//d가 1,2,3,4 각각 상,하,좌,우
	if (d == 1) {
		for (int r = 1; r < N; r++) {
			for (int c = 1; c <= N; c++) {
				GoUp(r, c);
