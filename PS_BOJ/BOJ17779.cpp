...
void assignRegiment(int x, int y, int d1, int d2) {//x,y는 5번선거구 최북단.
	...
	/*경계색칠*/
	//for (int r = x; r <= x + d1; r++) {//맨위에서 좌로 내려감
	//	for (int c = y; c >= y - d1; c--) {
	//		regiment[r][c] = 5;
	//	}
	//}
	//for (int r = x; r <= x + d2; r++) {//맨위에서 우로 내려감
	//	for (int c = y; c <= y + d2; c++) {
	//		regiment[r][c] = 5;
	//	}
	//}
	//for (int r = x + d1; r <= x + d1 + d2; r++) {//맨왼쪽에서 우로 내려감
	//	for (int c = y - d1; c <= y - d1 + d2; c++) {
	//		regiment[r][c] = 5;
	//	}
	//}
	//for (int r = x + d2; r <= x + d1 + d2; r++) {//맨오른쪽에서 좌로 내려감
	...
	/*속을 5로 색칠*/
	int str = x + 1; int stc = y;//플러드필 시작할 곳
	deque<pii> dqpii;
	regiment[str][stc] = 5;
	dqpii.push_back(mp(str, stc));
	while (!dqpii.empty()) {//BFS 플러드필
		int nowr = dqpii.front().ff; int nowc = dqpii.front().ss;
		dqpii.pop_front();
		for (int i = 0; i < 4; i++) {
			int rr = nowr + dr[i]; int cc = nowc + dc[i];
			if (rr > N || cc > N || rr < 1 || cc < 1 || regiment[rr][cc] == 5)
				continue;
			regiment[rr][cc] = 5;
			dqpii.push_back(mp(rr, cc));
		}
	}
